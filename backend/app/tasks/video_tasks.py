...
def process_video(self, video_id: str) -> dict:  # noqa: PLR0915
    ...
    try:
        ...
        # 4) Render clips: cut -> vertical reframe -> burn captions -> upload
        ...
        for idx, moment in enumerate(moments):
            ...
            try:
                ...
                # Build clip-relative segments for captions
                clip_segments = [
                    {
                        "start": max(0.0, float(s["start"]) - moment["start"]),
                        "end": max(0.1, float(s["end"]) - moment["start"]),
                        "text": s["text"],
                    }
                    for s in transcript.get("segments", [])
                    if float(s["end"]) > moment["start"] and float(s["start"]) < moment["end"]
                ]
                ...
                for lang in target_languages:
                    ...
                    try:
                        ...
                        dubs[lang] = {"s3_key": dub_key, "transcript": translated}
